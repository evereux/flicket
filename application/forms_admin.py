...
from .models import User, Group, group_maxlength, username_maxlength, name_maxlength, email_maxlength
...
def group_exists(form, field):
    ...
    """
    ok = True
    query = Group.query.filter_by(group_name=form.group_name.data).count()
    if query != 0:
...
class EditUserForm(FlaskForm):
    def __init__(self, *args, **kwargs):
        ...
        self.groups.choices = [(g.id, g.group_name) for g in Group.query.all()]
...
class AddGroupForm(FlaskForm):
    """ Add group form for admin section. """
